...
Nixarr to minimize manual post-deployment setup. It leverages `settings-sync` to
automatically configure Prowlarr, Sonarr, Radarr, and Bazarr on boot.
...
3. **Bazarr** will have its Sonarr and Radarr connections pre-configured, so it
   can immediately start managing subtitles.
...
application starts. These services use the application's REST API to apply your
Nix configuration. This means:
...
For settings-sync to work, authentication must be set to allow local API access:
